...
new MutationObserver(function(_ref,observer){var _ref2=_slicedToArray(_ref,1),mutation=_ref2[0];observer.disconnect();mutation.target.children[2].click();if(callback&&typeof callback==='function'){setTimeout(callback);}}).observe(e.parentElement,{attributes:false,childList:true,subtree:true});getBuyButton(e).click();};var buyTopItem=function buyTopItem(callback){Market_buyItem(getTopItemElement());};var buyAllItems=function buyAllItems(){var items=makeListItemIterator();var buyNextItem=function buyNextItem(){var _items$next=items.next(),item=_items$next.value,done=_items$next.done;var callback=done?undefined:buyNextItem;debugger;Market_buyItem(item,callback);};buyNextItem();};function Market(){var menuItems=[{text:'Buy Top Item',onClick:buyTopItem},{text:'Buy All Items',onClick:buyAllItems}];return react_default.a.createElement(src_Menu,{items:menuItems});}/* harmony default export */ var src_Market = (Market);
