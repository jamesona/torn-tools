...
new MutationObserver(function(_ref,observer){var _ref2=_slicedToArray(_ref,1),messageRow=_ref2[0].target;observer.disconnect();setTimeout(function(){if(messageRow.innerText.includes('Are you sure you want to buy')){messageRow.children[2].click();}else if(messageRow.innerText.includes('This item has already been purchased')){messageRow.nextElementSibling.click();}else{debugger;}if(callback&&typeof callback==='function'){setTimeout(callback);}});}).observe(e.parentElement,{attributes:false,childList:true,subtree:true});getBuyButton(e).click();};var buyTopItem=function buyTopItem(callback){Market_buyItem(getTopItemElement(),callback);};var buyAllItems=function buyAllItems(callback){var items=makeListItemIterator();var buyNextItem=function buyNextItem(){var _items$next=items.next(),item=_items$next.value,done=_items$next.done;console.log(done,item);if(item){Market_buyItem(item,done?callback:buyNextItem);}else if(callback&&typeof callback==='function'){callback();}};buyNextItem();};function Market(){var menuItems=[{text:'Buy Top Item',onClick:buyTopItem},{text:'Buy All Items',onClick:buyAllItems}];return react_default.a.createElement(src_Menu,{items:menuItems});}/* harmony default export */ var src_Market = (Market);
