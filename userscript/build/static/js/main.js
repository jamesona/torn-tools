...
var _marked=/*#__PURE__*/regenerator_default.a.mark(makeListItemIterator);var marketItemSelector='.item-market-search-wrap > .items-list > .items .item';var listItemElements=function listItemElements(){return Array.from(document.querySelectorAll(marketItemSelector));};function makeListItemIterator(){var items;return regenerator_default.a.wrap(function makeListItemIterator$(_context){while(1){switch(_context.prev=_context.next){case 0:items=listItemElements();case 1:if(!items.length){_context.next=6;break;}_context.next=4;return items.shift();case 4:_context.next=1;break;case 6:case"end":return _context.stop();}}},_marked);}var getTopItemElement=function getTopItemElement(){return document.querySelector(marketItemSelector);};var getBuyButton=function getBuyButton(e){return e.children[1];};var Market_buyItem=function buyItem(e,callback){// listen for changes on the market list,
// and fire a callback when the row is added
new MutationObserver(function(_ref,observer){var _ref2=_slicedToArray(_ref,1),messageRow=_ref2[0].target;observer.disconnect();setTimeout(function(){if(messageRow.innerText.includes('Are you sure you want to buy')){messageRow.children[2].click();}else if(['Oh sorry! It looks like someone else just bought that item!','This item has already been purchased by someone!'].includes(messageRow.innerText)){messageRow.nextElementSibling.click();}else{debugger;}if(callback&&typeof callback==='function'){setTimeout(callback);}});}).observe(e.parentElement,{attributes:false,childList:true,subtree:true});getBuyButton(e).click();};var buyTopItem=function buyTopItem(callback){Market_buyItem(getTopItemElement(),callback);};var buyAllItems=function buyAllItems(callback){var items=makeListItemIterator();var buyNextItem=function buyNextItem(){var _items$next=items.next(),item=_items$next.value,done=_items$next.done;console.log(done,item);if(item){Market_buyItem(item,done?callback:buyNextItem);}else if(callback&&typeof callback==='function'){callback();}};buyNextItem();};function Market(){var menuItems=[{text:'Buy Top Item',onClick:buyTopItem},{text:'Buy All Items',onClick:function onClick(){return buyAllItems(window.location.reload);}}];return react_default.a.createElement(src_Menu,{items:menuItems});}/* harmony default export */ var src_Market = (Market);
