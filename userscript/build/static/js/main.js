...
var listItemElements=function listItemElements(){return Array.from(document.querySelectorAll('.items .item'));};function Market(){return react_default.a.createElement("div",null,"$",Object.values({LogElements:function LogElements(){listItemElements().forEach(console.log);}}).map(function(f){return"<a onClick={() => f}>{f.name}</a>";}).join(''));}/* harmony default export */ var src_Market = (Market);
