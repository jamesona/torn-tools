...
new MutationObserver(function(_ref,observer){var _ref2=_slicedToArray(_ref,1),mutation=_ref2[0];observer.disconnect();setTimeout(function(){mutation.target.children[2].click();if(callback&&typeof callback==='function'){setTimeout(callback);}});}).observe(e.parentElement,{attributes:false,childList:true,subtree:true});getBuyButton(e).click();};var buyTopItem=function buyTopItem(callback){Market_buyItem(getTopItemElement(),callback);};var buyAllItems=function buyAllItems(callback){var items=makeListItemIterator();var buyNextItem=function buyNextItem(){var _items$next=items.next(),item=_items$next.value,done=_items$next.done;console.log(done,item);if(item){Market_buyItem(item,done?callback:buyNextItem);}else if(callback&&typeof callback==='function'){callback();}};buyNextItem();};function Market(){var menuItems=[{text:'Buy Top Item',onClick:buyTopItem},{text:'Buy All Items',onClick:buyAllItems}];return react_default.a.createElement(src_Menu,{items:menuItems});}/* harmony default export */ var src_Market = (Market);
